...
func BenchmarkVerify(b *testing.B) {
	...
	var seed [32]byte
	var msg [8]byte
	var sig [dilithium3scheme.SignatureSize]byte
	pk, sk := dilithium3scheme.NewKeyFromSeed(&seed)
	dilithium3scheme.SignTo(sk, msg[:], sig[:])
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		...
		dilithium3scheme.Verify(pk, msg[:], sig[:])
	}
}
...
func BenchmarkSign(b *testing.B) {
	...
	var msg [8]byte
	var sig [dilithium3scheme.SignatureSize]byte
	_, sk := dilithium3scheme.NewKeyFromSeed(&seed)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkGenerateKey(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		dilithium3scheme.NewKeyFromSeed(&seed)
	}
}
...
func TestSignThenVerifyAndPkSkPacking(t *testing.T) {
	var seed [dilithium3scheme.SeedSize]byte
	var sig [dilithium3scheme.SignatureSize]byte
	var msg [8]byte
	var pkb1, pkb2 [dilithium3scheme.PublicKeySize]byte
	var skb1, skb2 [dilithium3scheme.PrivateKeySize]byte
	...
	for i := uint64(0); i < 100; i++ {
		...
		for j := uint64(0); j < 10; j++ {
			...
			dilithium3scheme.SignTo(sk, msg[:], sig[:])
			if !dilithium3scheme.Verify(pk, msg[:], sig[:]) {
				t.Fatal()
			}
		}
		pk.Pack(&pkb1)
...
func TestPublicFromPrivate(t *testing.T) {
	...
	for i := uint64(0); i < 100; i++ {
		binary.LittleEndian.PutUint64(seed[:], i)
		pk, sk := dilithium3scheme.NewKeyFromSeed(&seed)
		pk2 := sk.Public().(*dilithium3scheme.PublicKey)
		var pkb1, pkb2 [dilithium3scheme.PublicKeySize]byte
		pk.Pack(&pkb1)
		pk2.Pack(&pkb2)
		if pkb1 != pkb2 {
