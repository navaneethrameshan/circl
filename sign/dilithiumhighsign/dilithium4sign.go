//package implements the signature scheme for Dilithium mode4
...
const (
	...
	SeedSize = mode4.SeedSize

	// PublicKeySize is the length in bytes of the packed public key.
	PublicKeySize = mode4.PublicKeySize

	// PrivateKeySize is the length in bytes of the packed public key.
	PrivateKeySize = mode4.PrivateKeySize

	// SignatureSize is the length in bytes of the signatures.
	SignatureSize = mode4.SignatureSize
)

// PublicKey is the type of Dilithium mode 4 public key.
type PublicKey struct {
	d mode4.PublicKey
}

// PrivateKey is the type of Dilithium mode 4 private key.
...
func (sk *PrivateKey) Sign(
	...
	if opts.HashFunc() != crypto.Hash(0) {
		return nil, errors.New("dilithium4: cannot sign hashed message")
	}
